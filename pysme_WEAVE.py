...
def setup_object(obj_ind):
    ...
    data releases. Everything called from params comes from the parameter file
    params.py located in the run directory.
...
#    data_path = './data'
    ...
    data_file_B = fits.open(path + params.spec_file_blue)

    # The green wavelength region
    data_file_G = fits.open(path + params.spec_file_green) 
...
#    obj_for_obs = 'WVE_19203729+3840075' # input("Object, please.")
    # input("Object, please.")
    obj_for_obs = data_file_G[2].data['CNAME'][int(obj_ind)] 

    field_for_obs = params.field_for_obs  # input ('lbol' | 'seis' | 'fixed')

    field_end = params.field_end

    vrad_flag = params.vrad_flag

    # Which segment of HR data to use, must be one of 
    # ('R' | 'B' | 'G' | '')
    wav_segment = params.wav_segment
...
#    setup_for_obs = "WEAVE_HR_R"  # input("Setup, please.")
    ...
    obs_name = str(vrad_flag) + "_" + obj_ind + "_" \
                    + str(obj_for_obs) + "_" + str(setup_for_obs) + "_" \
                    + field
...
#    iterations = [1, 2, 1, 20]
    ...
    return obs_name, obs_file, obj_for_obs, field_end, setup_for_obs, \
              field_for_obs, wav_segment, iterations, data_file_B, data_file_G
...
def set_atmosphere():
    ...
    atmosphere_grid_file = params.atmosphere_grid_file
...
#    line_list = 'master_hfs.lin'
    ...
    # atmosphere_abundance_grid = ['', '']
    atmosphere_abundance_grid = params.atmosphere_abundance_grid
...
def sme_variables():
    ...
    broad_lines:
        ...
        used in balmer_runs for Halpha and beta 
...
#    line_cores = []
    ...
    # things.
    # flag to specify how the radial velocity is computed
    vrad_flag = params.vrad_flag

    return broad_lines, depthmin, line_cores, vrad_flag


"""Part 2: read in spectra and extract relevant information"""


def get_wav_flux(obj_for_obs, wav_segment, data_file_B, data_file_G):
    """
    ...
    -------
        obj_for_obs: string
            CNAME of the object of interest
        wav_segment: string
            label for which wavelength ranges to analyze
        data_file_B: fits HDU
            WEAVE file containing data for the blue and red spectra
        data_file_G: fits HDU
            WEAVE file containing data for the green spectra

    Returns
    -------
        spec_wav: array_like
            Wavelength of the spectrum
        spec_flux: array_like
            Flux values of the spectrum
        spec_flux_err: array_like
            Flux error (inverse variance) for the spectrum
        data_ind: int
...
#    path = '/home/kryo6156/work/DATA/WEAVE/golden_sample/CCG_NGC6791_F1W1/HRB'
    ...
    # set up
    # The stacked L2 products lack some header information about the 
    # instrumental set up.
    head_file = fits.open(path + params.head_file)
...
#                    '/stacked_1002118__stacked_1002117_exp_b_APS.fits')
    ...
    data_ind_B_ext_4 = np.where(
                            data_file_B[4].data['CNAME'] == obj_for_obs)[0][0]
    data_ind_G_ext_4 = np.where(
                            data_file_G[4].data['CNAME'] == obj_for_obs)[0][0]

    # get index corresponding to the object id for L2 files
    data_ind_B_ext_5 = np.where(
                            data_file_B[5].data['CNAME'] == obj_for_obs)[0][0]
    data_ind_G_ext_5 = np.where(
                            data_file_G[5].data['CNAME'] == obj_for_obs)[0][0]

    # get index corresponding to the object id for L1 files
    head_ind = np.where(head_file[6].data['CNAME'] == obj_for_obs)[0][0]

    print('object (Blue)--> %s' \
            %data_file_B[5].data['CNAME'][data_ind_B_ext_5])
    print('object (Green)--> %s' \
            %data_file_G[5].data['CNAME'][data_ind_G_ext_5])
    print('gaia id --> %s' \
            %data_file_G[5].data['TARGID'][data_ind_B_ext_5])
    ...
    if wav_segment == "B_G_R":

        spec_wav = np.concatenate((
                      data_file_B[5].data['LAMBDA_RVS_B'][data_ind_B_ext_5,:],
                      data_file_G[5].data['LAMBDA_RVS_G'][data_ind_G_ext_5,:],
                      data_file_B[5].data['LAMBDA_RVS_R'][data_ind_B_ext_5,:]),
                         axis=None)
  
        spec_flux = np.concatenate((
                        data_file_B[5].data['FLUX_RVS_B'][data_ind_B_ext_5,:],
                        data_file_G[5].data['FLUX_RVS_G'][data_ind_G_ext_5,:],
                        data_file_B[5].data['FLUX_RVS_R'][data_ind_B_ext_5,:]),
                           axis=None)

        spec_flux_err = np.concatenate((
                        data_file_B[5].data['ERROR_RVS_B'][data_ind_B_ext_5,:],
                        data_file_G[5].data['ERROR_RVS_G'][data_ind_G_ext_5,:],
                        data_file_B[5].data['ERROR_RVS_R'][data_ind_B_ext_5,:]),
                            axis=None)

    elif wav_segment == "G_R":

        spec_wav = np.concatenate((
                      data_file_G[5].data['LAMBDA_RVS_G'][data_ind_G_ext_5,:],
                      data_file_G[5].data['LAMBDA_RVS_R'][data_ind_B_ext_5,:]),
                         axis=None)
  
        spec_flux = np.concatenate((
                        data_file_G[5].data['FLUX_RVS_G'][data_ind_G_ext_5,:],
                        data_file_G[5].data['FLUX_RVS_R'][data_ind_B_ext_5,:]),
                           axis=None)

        spec_flux_err = np.concatenate((
                        data_file_G[5].data['ERROR_RVS_G'][data_ind_G_ext_5,:],
                        data_file_G[5].data['ERROR_RVS_R'][data_ind_B_ext_5,:]),
                            axis=None)

    elif wav_segment == "R":

#        spec_wav = data_file_B[5].data['LAMBDA_RVS_R'][data_ind_B_ext_5,:]
#        spec_flux = data_file_B[5].data['FLUX_RVS_R'][data_ind_B_ext_5,:]
#        spec_flux_err =  data_file_B[5].data['ERROR_RVS_R'][data_ind_B_ext_5,:]

        spec_wav = data_file_G[5].data['LAMBDA_RVS_R'][data_ind_B_ext_5,:]
        spec_flux = data_file_G[5].data['FLUX_RVS_R'][data_ind_B_ext_5,:]
        spec_flux_err =  data_file_G[5].data['IVAR_RVS_R'][data_ind_B_ext_5,:]


    elif wav_segment == "test":

        spec_wav = \
            data_file_G[5].data['LAMBDA_RVS_R'][data_ind_B_ext_5,10365:10965]
        spec_flux = \
            data_file_G[5].data['FLUX_RVS_R'][data_ind_B_ext_5,10365:10965]
        spec_flux_err = \
            data_file_G[5].data['ERROR_RVS_R'][data_ind_B_ext_5,10365:10965]

    elif wav_segment == "test_green":

        spec_wav = \
            data_file_G[5].data['LAMBDA_RVS_G'][data_ind_B_ext_5][0:4500]
        spec_flux = \
                data_file_G[5].data['FLUX_RVS_G'][data_ind_B_ext_5][0:4500]
        spec_flux_err = \
                data_file_G[5].data['ERROR_RVS_G'][data_ind_B_ext_5][0:4500]

    return spec_wav, spec_flux, spec_flux_err, head_file, head_ind
...
#    resolution = spec_wav[:10] * res_fibre[head_ind]
    ...
    interpolation = interp1d(spec_wav, resolution)

    return interpolation, res_type

def doppler_wavelengths(spec_wav, vrad_global):                     
    """                                                                         
    # Adjusts the wavelengths according to the doppler shift and returns it.        

    Parameters
    -------
        total_ccd_wavelengths: array_like                                       
             All our wavelengths                                                 
        vrad_global: float                                                      
            The radial velocity of the star, used to adjust for its doppler     
            shifting of light                                                   
    Returns
    -------
       total_ccd_wavelength: array_like                                        
            New wavelengths adjusted for the doppler effect                     
    """                                                                         
    # Setting speed of light in km/s                                            
    c = 2.99792458E5                                                            
    # Shifts the wavelengths to account for the doppler effect.                 
    spec_wav_doppler = (spec_wav / (1 + (vrad_global / c)))               
                                                                               
    return spec_wav_doppler
...
def get_starting_params(data_file_G, obj_for_obs):
    """
    ...
    -------
        starting_params: dictionary
            A dictionary containing several parameters:
                ...
                macro_turb_vel
    """
    # fetch index according the the current object for this fits extension 
    # (the index is different than for the other extension containg the flux)
    data_ind_G_ext_2 = np.where(
                            data_file_G[2].data['CNAME'] == obj_for_obs)[0][0]

    # We use a dictionary to keep starting parameters in.
    starting_params = {}

    # extension 2 corresponds to the star table w/ all the RVS params
    starting_params['Teff'] = \
            data_file_G[2].data['TEFF_RVS'][data_ind_G_ext_2]
    starting_params['gravity'] = \
            data_file_G[2].data['LOGG_RVS'][data_ind_G_ext_2]
    starting_params['metallicity'] = \
            data_file_G[2].data['FEH_RVS'][data_ind_G_ext_2]
    starting_params['vrad_global'] = \
            data_file_G[2].data['VRAD'][data_ind_G_ext_2]
    starting_params['micro_turb_vel'] = \
            data_file_G[2].data['MICRO'][data_ind_G_ext_2]
    starting_params['rotational_vel'] = \
            data_file_G[2].data['VSINI_RVS'][data_ind_G_ext_2]
...
def microturbulence_macroturbulence(temperature, gravity):
    ...
    scrap the macro and just use vsini (rotational velocity)
    ...
    if ai1 == 0:
        print("Something wrong with microturbulence code and its if", \
                "requirements. Check it out --> pysme_WEAVE.py line 410.")
        exit()
    temperature_base = 5500
    gravity_base = 4.0
    # vmic =      ai1 + ai2*(t -t0) + ai3 * (t-t0)^2 + bi1 + bi2 * (g-g0) + 
    #       bi3 * (g-g0)^2 + ci1 + ci2 * z + ci3 * z^2
    # vmac = 3 * (aa1 + ba1*(t -t0) + ca1 * (t-t0)^2 + ba2*(g-g0) + 
    #       ca2*(g-g0)^2 + ba3*z + ca3*z^2) + 2.0
...
def update_mode_info(field_end, field_for_obs, data_file, data_ind):
    ...
    If Seis, prepare asteroseismic info, if LBOL, prepare 
    photomettric/astrometric info, if Lbol/Seis, update logG based on provided 
    info.
    Parameters
    -------
        field_end: 
            Type of mode. LBOL, SEIS, FIXED
        field_for_obs: 
            Checks for if it is the sun. Requires special measures for if it 
            is.
        reduction_and_analysis_data: 
            Information on the star to produce the astromtric/asteroseismic 
            information.
    Returns
    -------
        reduction_variable_dict: 
            Contains the astromtric/asteroseismic data such as distance, 
            parallax, magnitude of k band, and more.
    """
    # Dictionary to put the variables in.
    reduction_variable_dict = {}

    # Elif just running in lbol mode, but not the sun we use a mnormal lbol 
    # update.
    ...
    # If SEIS mode: get numax. A separate method to lbol to retrieve required 
    # astroseismic data.
    # Numax: The value of frequency at which the oscillation displays its 
    # strongest pulsation amplitude.
