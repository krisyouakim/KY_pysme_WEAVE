...
#    field_end = "lbol"
    ...
    # folder.
    setup_for_obs = "APS"  # input("Setup, please.")

    # Name of the file that will be saved.
    obs_name = str(obj_for_obs) + "_" + str(field_for_obs) + "_" + \
                   str(setup_for_obs) + "_Sp"

    # Path to the data file containing the WEAVE spectrum
    data_path = './data/20160903'
...
def get_wav_flux(obj_for_obs):
    ...
    -------
        ...
        obj_ind: the index corresponding to the object id
        data_file: fits_hdu
...
#    spectra_file = fits.open(path + 'stacked_1003798_1003797.aps.fits')
    data_file = fits.open(
#                    'data/20160903/5_rows_stacked_1002046_1002045.aps.fits')
                    'data/20160903/stacked_1002046_1002045.aps.fits')


    # get index corresponding to the object id
    obj_ind = np.where(data_file[4].data['CNAME'] == obj_for_obs)[0][0]

    print('object --> %s' %data_file[4].data['CNAME'][obj_ind])

    """#####Add in a catch for wrong file names or non single objets#####"""
    # The fits file contains 9 header units: 1 - FIBTABLE, 2 - CLASS_SPECTRA
    # 3 - STELLAR_SPECTRA_FERRE, 4 - STELLAR_SPECTRA_RVS, 5 - GALAXY_SPECTRA
    # 6 - CLASS_TABLE, 7 - stella_table, 8 - STELLAR_TABLE_RVS,
    # 9 - GALAXY_TABLE. 
    # The unormalized spectra are in number 4.

    spec_wav = data_file[4].data['LAMBDA_STELLAR'][obj_ind,:]
    spec_flux = data_file[4].data['FLUX_STELLAR'][obj_ind,:]
    spec_flux_err =  data_file[4].data['IVAR_STELLAR'][obj_ind,:]

    return spec_wav, spec_flux, spec_flux_err, data_file, obj_ind

def get_resolution(obj_for_obs, data_file, spec_wav, obj_ind):
    """
    ...
    res_type = data_file[0].header['MODE']

    # resolution of the observed fibre
    res_fibre = data_file[1].data['RESOL']
    ...
    resolution = spec_wav[:10] * res_fibre[obj_ind]

    interpolation = interp1d(spec_wav, spec_wav * res_fibre[obj_ind])
...
def get_starting_params(data_file, obj_ind):
    """
    ...
    starting_params['Teff'] = data_file[8].data['TEFF_RVS'][obj_ind]
    starting_params['gravity'] = data_file[8].data['LOGG_RVS'][obj_ind]
    starting_params['metallicity'] = data_file[8].data['FEH_RVS'][obj_ind]
    starting_params['vrad_global'] = data_file[8].data['VRAD_RVS'][obj_ind]
    starting_params['micro_turb_vel'] = data_file[7].data['MICRO'][obj_ind]
    starting_params['rotational_vel'] = data_file[8].data['VSINI_RVS'][obj_ind]
...
def update_mode_info(field_end, field_for_obs, data_file, obj_ind):
    """
    Input:
...
def get_gaia_params(data_file, obj_ind):
    """
    ...
    spec_table = data_file[1].data[obj_ind]
    ...
    cbj_dict = pysme_gaia_dist_extinc.get_cbj_dist(data_file, obj_ind)
