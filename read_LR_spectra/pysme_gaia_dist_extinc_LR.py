...
def get_cbj_dist(data_file, obj_ind):
    """
...
def get_extinction(cbj_dict, data_file, obj_ind):
    """
...
#    gaia_id=4144473463028744832
    ...
    #gaia_id=2175048202654984832
    #gaia_id=2175058991613000960
    gaia_G_mag = float(data_file[1].data['MAG_GG'][0])
    gaia_BP_RP = (float(data_file[1].data['MAG_BP'][0]) 
                   - float(data_file[1].data['MAG_RP'][0]))

    targ_ra = float(data_file[1].data['TARGRA'][0])
    targ_dec = float(data_file[1].data['TARGDEC'][0])
    ...
    #just for testing
    targ_ra = float(r['ra'].data[0])
    targ_dec = float(r['dec'].data[0])
    gaia_BP_RP = float(r['bp_rp'].data[0])
    gaia_G_mag = float(r['phot_g_mean_mag'].data[0])
    ...
    if len(r) == 0:
        ...
        if float(r['phot_g_mean_mag'].data[0]) - gaia_G_mag > 0.5:
            ...
            extinction_dict['bprp_red'] = '--'
            extinction_dict['bprp_red_lower'] = '--'
            extinction_dict['bprp_red_upper'] = nan
